...
public class ConstructorClassSyntaxModel : ILabeledMembersSyntaxModel, ILabeledMembersSemanticModel
{
    public TypeDeclarationSyntax ClassDeclaration { get; }
    private readonly InitalizedMemberList members = new();
    ...
    public void AddMember(SyntaxNode declSyntax) => members.AddMember(declSyntax);


    public ILabeledMembersSemanticModel AddSemanticInfo(SemanticModel semanticModel)
    {
        members.FindParentConstructors(SymbolForTargetClass(semanticModel));
        return this;
    } 
    ...
    public void GenerateCode(CodeWriter cw)
    {
        ...
        foreach (var constructor in members.Constructors.DefaultIfEmpty(Array.Empty<MemberData>()))
        {
    ...
    private void GenerateSingleConstructor(CodeWriter cw, IList<MemberData> constructorParams)
    {
        ...
        cw.Append("(");
        GenerateParameters(cw, constructorParams.Concat(members.FieldsAndProperties));
        cw.Append(")");
    ...
    {
        foreach (var member in members.FieldsAndProperties)
        {
