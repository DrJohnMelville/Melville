...
namespace Melville.TestHelpers.Assertions
{
    ...
    {
        private static readonly JsonStringComparer jsonStringComparer = new();
    ...
    public class JsonStringComparer : IEqualityComparer<string>
    {
        private static readonly JsonElementComparer innerComp = new();

        public bool Equals(string? x, string? y)
        {
            return x != null && y != null && innerComp.Equals(StringToElement(x), StringToElement(y));
        }

        public int GetHashCode(string obj) => innerComp.GetHashCode(StringToElement(obj));

        private static JsonElement StringToElement(string obj) => JsonDocument.Parse(obj, new JsonDocumentOptions()
        {
